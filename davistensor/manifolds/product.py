...
class ProductManifold(Manifold):
    ...
    def __repr__(self) -> str:
        return f"ProductManifold({self.name})"
    ...
    def norm(self, x: np.ndarray, v: np.ndarray) -> float:
        """
        Norm is sqrt of inner product.
        """
        return np.sqrt(max(0, self.inner(x, v, v)))
...
def HyperbolicSphere(hyp_dim: int, sphere_dim: int, c: float = 1.0) -> ProductManifold:
    """
    ...
    return ProductManifold([
        Hyperbolic(hyp_dim, c=c),
        Sphere(sphere_dim)
    ])


def HyperbolicEuclidean(hyp_dim: int, euc_dim: int, c: float = 1.0) -> ProductManifold:
    """
...
def MultiHyperbolic(dim: int, n_copies: int, curvatures: Optional[List[float]] = None) -> ProductManifold:
    ...
    return ProductManifold([
        Hyperbolic(dim, c=c) for c in curvatures
    ])
...
def test_product():
    ...
    print(f"   HyperbolicSphere(4, 3): {hs.name}")
    assert hs.check_point(hs.random_point())
    
    he = HyperbolicEuclidean(4, 3)
    print(f"   HyperbolicEuclidean(4, 3): {he.name}")
    assert he.check_point(he.random_point())
    
    mh = MultiHyperbolic(3, 3)
    print(f"   MultiHyperbolic(3, 3): {mh.name}")
    assert mh.check_point(mh.random_point())
    
    ms = MultiSphere(2, 2)
    print(f"   MultiSphere(2, 2): {ms.name}")
    assert ms.check_point(ms.random_point())
