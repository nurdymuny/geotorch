"""
DavisTensor SPD Manifold Specification
======================================
...
class SPD(Manifold):
    ...
    def ambient_dim(self) -> int:
        """Ambient dimension = n² (full matrices, but we use n×n shape)."""
    ...
    def _eigvalsh(self, P: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _sqrtm(self, P: np.ndarray) -> np.ndarray:
        """
        ...
        λ_sqrt = np.sqrt(np.maximum(λ, 1e-10))  # Clamp for stability
        return V @ (λ_sqrt[..., None] * np.swapaxes(V, -2, -1))
    
    def _sqrtm_inv(self, P: np.ndarray) -> np.ndarray:
        """
        ...
        """
        λ, V = self._eigvalsh(P)
        λ_inv_sqrt = 1.0 / np.sqrt(np.maximum(λ, 1e-10))
        return V @ (λ_inv_sqrt[..., None] * np.swapaxes(V, -2, -1))
    
    def _logm(self, P: np.ndarray) -> np.ndarray:
        """
        Matrix logarithm.
        ...
        """
        λ, V = self._eigvalsh(P)
        λ_log = np.log(np.maximum(λ, 1e-10))
        return V @ (λ_log[..., None] * np.swapaxes(V, -2, -1))
    ...
    def _expm(self, A: np.ndarray) -> np.ndarray:
        ...
        λ_exp = np.exp(λ)
        return V @ (λ_exp[..., None] * np.swapaxes(V, -2, -1))
    
    def _powm(self, P: np.ndarray, power: float) -> np.ndarray:
        """
        Matrix power.
        ...
        """
        λ, V = self._eigvalsh(P)
        λ_pow = np.power(np.maximum(λ, 1e-10), power)
        return V @ (λ_pow[..., None] * np.swapaxes(V, -2, -1))
    ...
    def random_point(self, *shape: int) -> np.ndarray:
        ...
        # Random matrix
        A = np.random.randn(*full_shape)
        
        # P = A @ A^T + εI
        P = A @ np.swapaxes(A, -2, -1)
        P = P + 0.1 * np.eye(n)  # Ensure positive definite
    ...
    def origin(self, *shape: int) -> np.ndarray:
        ...
        I = np.eye(n)
        if shape:
            return np.broadcast_to(I, (*shape, n, n)).copy()
        return I
    
    def check_point(self, P: np.ndarray, atol: float = 1e-5) -> bool:
        """
        Check if P is symmetric positive definite.
        """
        # Check symmetry
        if not np.allclose(P, np.swapaxes(P, -2, -1), atol=atol):
            return False
    ...
    def project_point(self, P: np.ndarray) -> np.ndarray:
        ...
        λ_clamped = np.maximum(λ, 1e-6)
        
        # Reconstruct
        return V @ (λ_clamped[..., None] * np.swapaxes(V, -2, -1))
    
    # =========================================================================
    # Tangent Space Operations
    # =========================================================================
    
    def check_tangent(self, P: np.ndarray, V: np.ndarray, atol: float = 1e-5) -> bool:
        """
        Tangent vectors are symmetric matrices.
        """
        return np.allclose(V, np.swapaxes(V, -2, -1), atol=atol)
    ...
    def random_tangent(self, P: np.ndarray) -> np.ndarray:
        """
    ...
    def metric(self, P: np.ndarray) -> np.ndarray:
        """
        Metric tensor at P.
    ...
    def inner(self, P: np.ndarray, U: np.ndarray, V: np.ndarray) -> np.ndarray:
        """
        ...
        return np.trace(temp, axis1=-2, axis2=-1)
    
    def norm(self, P: np.ndarray, V: np.ndarray) -> np.ndarray:
        """
        ||V||_P = sqrt(tr(P^{-1} V P^{-1} V))
        """
        return np.sqrt(np.maximum(self.inner(P, V, V), 0))
    ...
    def log(self, P: np.ndarray, Q: np.ndarray) -> np.ndarray:
        """
    ...
    def distance(self, P: np.ndarray, Q: np.ndarray) -> np.ndarray:
        """
        ...
        # Eigenvalues of M
        λ = np.linalg.eigvalsh(M)
        
        # Distance = sqrt(sum(log(λ)²))
        log_λ = np.log(np.maximum(λ, 1e-10))
        return np.sqrt(np.sum(log_λ ** 2, axis=-1))
    ...
    def frechet_mean(
        ...
        mean = self.project_point(np.tensordot(weights, points, axes=1))
...
def test_spd():
    ...
    assert np.allclose(norm_P, norm_Q, rtol=1e-3)
    print("   ✅ PASS")
